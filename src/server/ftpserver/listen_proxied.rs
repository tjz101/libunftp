            ...
            tokio::select! {
                ...
                Ok((tcp_stream, _socket_addr)) = listener.accept() => {
                    ...
                    if destination_port == self.external_control_port {
                        ...
                        let result = controlchan::spawn_loop::<Storage,User>(params, tcp_stream, Some(source), Some(proxyloop_msg_tx.clone()), self.client_charset, self.shutdown_topic.subscribe().await).await;
                        if result.is_err() {
